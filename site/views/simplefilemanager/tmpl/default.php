...
?>
	...
	<?php if ($this->item) : ?>
			...
			<table class="table">
				...
				<?php if($this->showLicence): ?>
					<tr>
						...
						<td><?php if( $this->item->license_link ) echo '<a href="'.$this->item->license_link.'" target="_blank">'.$this->item->license.'</a>'; else echo $this->item->license;?></td>
					</tr>
				<?php endif; ?>

				<tr>
					<th><?php echo JText::_('COM_SIMPLEFILEMANAGER_HEADING_VERSION'); ?></th>
					<td><?php if( $this->item->version ) echo $this->item->version; ?></td>
				</tr>
				...
				<?php if($this->item->canDownload): ?>
	         	<tr>
            		...
            		<td>
							...
							<?php else: ?>
								<a class="btn" href="<?php echo JRoute::_('index.php?option=com_simplefilemanager&task=download&id='.(int)$this->item->id); ?>">
	    ...
	    <?php if($canEdit && $this->item->checked_out == 0): ?>
			<a class="btn" href="<?php echo JRoute::_('index.php?option=com_simplefilemanager&task=simplefilemanager.edit&id='.$this->item->id); ?>"><?php echo JText::_("COM_SIMPLEFILEMANAGER_EDIT_ITEM"); ?></a>
		<?php endif; ?>

		<?php if(JFactory::getUser()->authorise('core.delete','com_simplefilemanager')):?>
			<a class="btn" href="<?php echo JRoute::_('index.php?option=com_simplefilemanager&task=simplefilemanager.remove&id=' . $this->item->id, false, 2); ?>"><?php echo JText::_("COM_SIMPLEFILEMANAGER_DELETE_ITEM"); ?></a>
		<?php endif; ?>
